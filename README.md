...
```bash
# 1. 克隆仓库
git clone https://github.com/YOUR_USERNAME/ai-submission-analyzer.git
...
Windows 用户可直接双击 `start.bat` 一键启动（自动创建虚拟环境并安装依赖）。
...
├── .gitignore
├── tampermonkey-collector.js # 油猴脚本（网页内容采集）
├── usage_guide.md            # 详细使用说明
├── project_doc.md            # 项目架构文档
├── data/                     # 运行时数据（自动生成）（被 gitignore）
│   ├── history.json          #   分析历史记录
│   ├── api_keys.json         #   已保存的 API 密钥
│   └── categories.json       #   自定义分类配置
└── output/                   # 导出文件目录（被 gitignore）
```
...
┌─────────────────────────────────────────┐
│          表现层 (Presentation)            │
│  app.py (Gradio UI) + CSS 暗色主题        │
├─────────────────────────────────────────┤
│        业务逻辑层 (Business Logic)         │
│  ai_engine.py (标题→分类→摘要→评估)        │
├─────────────────────────────────────────┤
│         AI 服务层 (AI Service)            │
│  config.py + OpenAI SDK (统一接口)        │
│  DeepSeek / 通义千问 / 智谱GLM / Kimi     │
├─────────────────────────────────────────┤
│         数据输出层 (Data Output)           │
│  report_generator.py (Markdown/TXT/Word/PDF) │
└─────────────────────────────────────────┘
```
...
| **浏览器扩展** | Tampermonkey | 网页内容采集 |

---

## 🌐 油猴脚本（网页采集）

项目包含一个 Tampermonkey 脚本 `tampermonkey-collector.js`，可在浏览微博、知乎、百度热搜等网页时一键采集内容。

### 安装方法

1. 安装 [Tampermonkey](https://www.tampermonkey.net/) 浏览器扩展
2. 点击 Tampermonkey → 添加新脚本
3. 将 `tampermonkey-collector.js` 内容粘贴进去
4. 保存（Ctrl+S）

### 支持平台

微博 · 知乎 · 百度热搜 · 抖音 · 小红书 · B站 · 通用网页提取
...
MIT License
>>>>>>> c5a2f2cb715c4c200d296656ef2d3d7956879230
